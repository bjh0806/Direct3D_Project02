...
void CPlayer::Move(DWORD dwDirection, float fDistance, bool bUpdateVelocity) {
	...
	if (dwDirection)
	{
		...
		if (m_xmf3Position.z >= 0 && m_xmf3Position.z <= 2056) {
			if (dwDirection & DIR_FORWARD) xmf3Shift = Vector3::Add(xmf3Shift, m_xmf3Look, fDistance);
			if (dwDirection & DIR_BACKWARD) xmf3Shift = Vector3::Add(xmf3Shift, m_xmf3Look, -fDistance);
		}
		
		if (m_xmf3Position.x >= 0 && m_xmf3Position.x <= 2056) {
			if (dwDirection & DIR_RIGHT) xmf3Shift = Vector3::Add(xmf3Shift, m_xmf3Right, fDistance);
			if (dwDirection & DIR_LEFT) xmf3Shift = Vector3::Add(xmf3Shift, m_xmf3Right, -fDistance);
		}
		
		if (m_xmf3Position.y <= 500.0f) {
...
CTerrainPlayer::CTerrainPlayer(ID3D12Device *pd3dDevice, ID3D12GraphicsCommandList *pd3dCommandList, ID3D12RootSignature *pd3dGraphicsRootSignature, void *pContext, int nMeshes) : CPlayer(nMeshes) {
	...
	CAirplaneMeshDiffused* pCubeMesh = new CAirplaneMeshDiffused(pd3dDevice, pd3dCommandList, 20.0f, 4.0f, 8.0f);
	SetMesh(0, pCubeMesh);
